...
async function carregarRefeicoesDodia() {
    ...
    try {
        const response = await fetch(`http://127.0.0.1:8080/api/refeicoes/dia?usuario_id=${usuarioId}`);
...
confirmDeleteBtn.addEventListener('click', async function() {
    ...
    try {
        const response = await fetch(`http://127.0.0.1:8080/api/refeicoes/${mealToDeleteId}`, {
            method: 'DELETE'
        });
